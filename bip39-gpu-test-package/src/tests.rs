    ...
    fn testgpu() {
        ...
        match GpuAccelerator::new() {
            Ok(gpu) => {
                ...
                match get_gpu_info() {
                    Ok(info) => {
                        println!("✓ GPU Device Info: {}", info);
                    }
                    Err(e) => {
                        println!("⚠ Could not get GPU info: {}", e);
                    }
                }
                
                // Create a realistic test configuration with constraints
                let test_config = Config {
                    word_constraints: vec![
                        ...
                        WordConstraint {
                            position: 11,
                            prefix: None,
                            words: vec!["about".to_string(), "above".to_string(), "abuse".to_string()],
                        },
                    ],
                    ethereum: EthereumConfig {
                        derivation_path: "m/44'/60'/0'/0/0".to_string(),
                ...
                println!("✓ Word space generated:");
                println!("  - Total combinations: {}", word_space.total_combinations);
                println!("  - Position 0: {} words (constrained)", word_space.positions[0].len());
                println!("  - Position 11: {} words (constrained)", word_space.positions[11].len());
                for i in 1..11 {
                    if i != 11 {
                        println!("  - Position {}: {} words", i, word_space.positions[i].len());
                    }
                }
                ...
                // Convert word space to constraints for GPU
                ...
                println!("🔄 Testing GPU batch processing (small batch)...");
                let start_time = Instant::now();
                match gpu.process_batch(0, 100, &target_address, &constraints) {
                    Ok(result) => {
                        let elapsed = start_time.elapsed();
                        match result {
                            Some((index, mnemonic)) => {
                                println!("🎉 GPU found result: index={}, mnemonic={}", index, mnemonic);
                                println!("✓ GPU processing time: {:?}", elapsed);
                            }
                            None => {
                                println!("✓ GPU processed small batch successfully (no match found in test range)");
                                println!("✓ GPU processing time: {:?} for 100 candidates", elapsed);
                                let rate = 100.0 / elapsed.as_secs_f64();
                                println!("✓ GPU processing rate: {:.1} mnemonics/sec", rate);
                            }
                        }
                        
                        // Test with a medium batch for performance measurement
                        println!("🔄 Testing GPU performance (medium batch)...");
                        let start_time = Instant::now();
                        match gpu.process_batch(0, 1000, &target_address, &constraints) {
                            Ok(result) => {
                                ...
                                match result {
                                    Some((index, mnemonic)) => {
                                        println!("🎉 GPU found result in medium batch: index={}, mnemonic={}", index, mnemonic);
                                    }
                                    None => {
                                        println!("✓ GPU processed medium batch successfully (no match found)");
                                    }
                                }
                                println!("✓ GPU processing time: {:?} for 1000 candidates", elapsed);
                                println!("✓ GPU processing rate: {:.1} mnemonics/sec", rate);
                                
                                // Compare with expected CPU rate
                                let cpu_rate = 17800.0; // Expected CPU rate
                                let speedup = rate / cpu_rate;
                                if speedup > 1.0 {
                                    println!("🚀 GPU speedup: {:.1}x faster than CPU", speedup);
                                } else {
                                    println!("⚠ GPU rate: {:.1}x of CPU rate (may need optimization)", speedup);
                                }
                            }
                            Err(e) => {
                                println!("⚠ GPU processing error on medium batch: {}", e);
                            }
                        }
                    }
                    Err(e) => {
                        println!("⚠ GPU processing error: {}", e);
                        println!("This may indicate a kernel compilation or execution issue");
                    }
                }
            ...
            Err(e) => {
                ...
                if e.to_string().contains("CL_PLATFORM_NOT_FOUND_KHR") {
                    println!("💡 Install OpenCL drivers:");
                    println!("   NVIDIA: Install CUDA toolkit");
                    println!("   AMD: Install ROCm or AMD APP SDK");
                    println!("   Intel: Install Intel OpenCL runtime");
                }
