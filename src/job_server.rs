...
impl JobServer {
    ...
    pub fn complete_job(&self, completion: &JobCompletion) -> Result<(), ApiError> {
        ...
        let job = jobs.get_mut(&completion.job_id)
            .ok_or_else(|| ApiError {
        ...
        if completion.success {
            job.mark_completed();
            println!("Job {} completed by worker {} ({} mnemonics checked)", 
            ...
            let error = completion.error.clone().unwrap_or_else(|| "Unknown error".to_string());
            job.mark_failed(error);
            println!("Job {} failed on worker {}: {}", 
                completion.job_id, completion.worker_id, 
                completion.error.as_ref().unwrap_or(&"Unknown error".to_string()));
        }
    ...
    pub fn update_heartbeat(&self, heartbeat: &WorkerHeartbeat) -> Result<(), ApiError> {
        ...
        let job = jobs.get_mut(&heartbeat.job_id)
            .ok_or_else(|| ApiError {
                error: "Job not found".to_string(),
                code: 404,
            })?;
    ...
    pub fn get_status(&self) -> ServerStatus {
        ...
        let pending_jobs = jobs.values().filter(|job| job.status == JobStatus::Pending).count();
        let assigned_jobs = jobs.values().filter(|job| matches!(job.status, JobStatus::Assigned { .. })).count();
        let completed_jobs = jobs.values().filter(|job| job.status == JobStatus::Completed).count();
        let failed_jobs = jobs.values().filter(|job| matches!(job.status, JobStatus::Failed { .. })).count();
        
        // Calculate combinations searched
        let combinations_searched: u128 = jobs.values()
            .filter(|job| job.status == JobStatus::Completed)
            .map(|job| job.size())
            .sum();
        
        // Get unique active workers
        let active_workers: std::collections::HashSet<String> = jobs.values()
            .filter_map(|job| {
    ...
    pub fn start_timeout_handler(&self) {
        ...
        thread::spawn(move || {
            loop {
                ...
                let timed_out_jobs: Vec<JobId> = jobs.values()
                    .filter(|job| {
                        matches!(job.status, JobStatus::Assigned { .. }) && job.is_timed_out(timeout_seconds)
                    })
