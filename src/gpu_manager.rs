...
impl GpuManager {
    ...
    pub fn from_config(config: &Config) -> Result<Self, Box<dyn Error>> {
        let gpu_config = config.gpu.as_ref().unwrap_or(&GpuConfig {
        ...
        if !backend.is_available() {
            return Err(format!("GPU backend '{}' is not available on this system", gpu_config.backend).into());
        }
        ...
        println!("GPU Manager initialized with {} backend and {} device(s)", 
                 backend.backend_name(), devices.len());
